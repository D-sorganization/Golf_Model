...
function handles = createModelingPanel(parent, handles)
    ...
    % Detailed explanation of modeling approach
    modeling_text = sprintf(['Modeling Approach:\n', ...
                            '• Variable Torques: Random coefficients A-G (±range)\n', ...
                            '• Zero Torque: All coefficients = 0 (passive motion)\n', ...
                            '• Constant Torque: G=constant, A-F=0 (steady force)']);
    
    uicontrol('Parent', modeling_panel, ...
              'Style', 'text', ...
              'String', modeling_text, ...
...
function handles = createTrialTablePanel(parent, handles)
    ...
    % Trial table replaced with MATLAB-only listbox (NO JAVA!)
    uicontrol('Parent', trial_table_panel, ...
              'Style', 'text', ...
              'String', 'Trial Results (Trial# | Status | Data Points | Duration)', ...
              'Position', [10, 280, 620, 20], ...
              'HorizontalAlignment', 'left', ...
              'FontSize', 10, ...
              'FontWeight', 'bold');
    
    handles.trial_table = uicontrol('Parent', trial_table_panel, ...
                                   'Style', 'listbox', ...
                                   'Position', [10, 50, 630, 220], ...
                                   'FontSize', 9, ...
                                   'String', {'No trials generated yet', 'Click "Generate Dataset" to start'}, ...
                                   'BackgroundColor', [1, 1, 1], ...
                                   'Callback', @trialTableSelectionCallback);
    ...
    if ~isempty(selected_row)
        ...
        if ~isempty(handles.trial_table_data) && selected_row <= size(handles.trial_table_data, 1)
            ...
            if length(trial_data) >= 5 && ~isempty(trial_data{5})
                % Coefficient display replaced with MATLAB-only listbox (NO JAVA!)
                uicontrol('Parent', detail_fig, ...
                          'Style', 'text', ...
                          'String', 'Polynomial Coefficients:', ...
                          'Position', [20, y_pos - 20, 200, 20], ...
                          'FontSize', 10, ...
                          'FontWeight', 'bold', ...
                          'HorizontalAlignment', 'left');
                
                coeff_display = uicontrol('Parent', detail_fig, ...
                                         'Style', 'listbox', ...
                                         'Position', [20, y_pos - 170, 660, 140], ...
                                         'FontSize', 10, ...
                                         'BackgroundColor', [1, 1, 1]);
                
                % Parse coefficients string and create listbox data (MATLAB-only!)
                coeff_str = trial_data{5};
                coeff_data = parseCoefficientsString(coeff_str);
                
                % Convert table data to listbox strings
                if ~isempty(coeff_data)
                    listbox_strings = {};
                    for i = 1:size(coeff_data, 1)
                        % Format: "Coefficient: Value - Description"
                        listbox_strings{i} = sprintf('%s: %s - %s', ...
                            coeff_data{i,1}, coeff_data{i,2}, coeff_data{i,3});
                    end
                    set(coeff_display, 'String', listbox_strings);
                else
                    set(coeff_display, 'String', {'No coefficient data available'});
                end
            end
