function GolfSwingDataGenerator()
    % GolfSwingDataGenerator - GUI for generating golf swing training data
    ...
    % Initialize preview only if handles are properly set up
    if isfield(handles, 'preview_table') && isfield(handles, 'num_trials_edit')
...
function handles = createMainLayout(fig, handles)
    ...
    % Title panel - positioned within figure bounds
    ...
    % Left column - properly sized within figure
    left_panel = uipanel('Parent', fig, ...
                        'Position', [0.01, 0.01, 0.48, 0.91], ...
                        'BackgroundColor', [0.96, 0.96, 0.96], ...
                        'BorderType', 'line');
    
    % Left scroll bar - positioned within left panel
    left_scroll = uicontrol('Parent', left_panel, ...
                           'Style', 'slider', ...
                           'Units', 'normalized', ...
                           'Position', [0.95, 0.01, 0.04, 0.98], ...
                           'Min', 0, 'Max', 1, ...
                           'Value', 1, ...
                           'Callback', @(src,evt) scrollLeftColumn(src, evt, handles));
    
         % Left content panel - within left panel bounds
     left_content = uipanel('Parent', left_panel, ...
                           'Position', [0.01, 0.01, 0.90, 0.98], ...
                           'BackgroundColor', [0.96, 0.96, 0.96], ...
                           'BorderType', 'none');
    
    % Right column - properly sized within figure
    right_panel = uipanel('Parent', fig, ...
                         'Position', [0.51, 0.01, 0.48, 0.91], ...
                         'BackgroundColor', [0.96, 0.96, 0.96], ...
                         'BorderType', 'line');
    
    % Right scroll bar - positioned within right panel
    right_scroll = uicontrol('Parent', right_panel, ...
         ...
         % Right content panel - within right panel bounds
...
function handles = createLeftColumnContent(parent, handles)
    % Create all content for the left column with proper spacing
...
function handles = createRightColumnContent(parent, handles)
    % Create all content for the right column with proper spacing
...
function handles = createTrialAndDataPanel(parent, handles)
    ...
    uicontrol('Parent', panel, ...
         ...
         handles.num_trials_edit = uicontrol('Parent', panel, ...
                                        ...
                                        'FontSize', 10, ...
                                        'Callback', @(src,evt) safeUpdateCoefficientsPreview(src, evt, handles));
...
function handles = browseOutputFolder(src, ~, handles)
 ...
 function safeUpdateCoefficientsPreview(src, evt, handles)
     if isfield(handles, 'coefficients_table') && isfield(handles, 'num_trials_edit')
         updateCoefficientsPreview(src, evt, handles);
     end
 end

 function handles = updatePreview(~, ~, handles)
     try
         % Check if required handles exist
         if ~isfield(handles, 'num_trials_edit') || ~isfield(handles, 'preview_table')
             return;
         end
 ...
 function handles = updateCoefficientsPreview(~, ~, handles)
     try
         % Check if required handles exist
         if ~isfield(handles, 'num_trials_edit') || ~isfield(handles, 'coefficients_table')
             return;
         end
