...
function handles = createJointEditorPanel(parent, handles)
    ...
    handles.trial_selection_popup = uicontrol('Parent', panel, ...
                                             'Style', 'popupmenu', ...
                                             'String', {'All Trials', 'Trial 1', 'Trial 2', 'Trial 3', 'Trial 4', 'Trial 5'}, ...
...
function updateJointCoefficients(src, evt, handles)
    try
        ...
        % Update trial selection dropdown based on current table
        table_data = get(handles.coefficients_table, 'Data');
        if ~isempty(table_data)
            num_trials = size(table_data, 1);
            trial_options = {'All Trials'};
            for i = 1:min(num_trials, 20) % Limit to 20 trials in dropdown
                trial_options{end+1} = sprintf('Trial %d', i);
            end
            set(handles.trial_selection_popup, 'String', trial_options);
        end
...
function applyJointToTable(src, evt, handles)
    try
        ...
        trial_options = get(handles.trial_selection_popup, 'String');
        selected_trial_option = trial_options{trial_selection_idx};
        ...
        if trial_selection_idx == 1 % All Trials
            ...
            target_trial = trial_selection_idx - 1; % Adjust for "All Trials" at index 1
            if target_trial <= size(table_data, 1)
                target_rows = target_trial;
            else
                errordlg('Selected trial does not exist in table.', 'Error');
                return;
            end
        end
...
function loadJointFromTable(src, evt, handles)
    try
        ...
        if trial_selection_idx == 1 % All Trials - use first trial
            ...
            source_row = trial_selection_idx - 1; % Adjust for "All Trials" at index 1
            if source_row > size(table_data, 1)
                errordlg('Selected trial does not exist.', 'Error');
                return;
            end
        end
