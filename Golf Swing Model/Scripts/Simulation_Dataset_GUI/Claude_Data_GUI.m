function Data_Generation_GUI_Fixed()
    % GolfSwingDataGenerator - Modern GUI for generating golf swing training data
    % This is the fully fixed version with all errors resolved
    
    % Modern color scheme
    colors = struct();
    colors.primary = [0.2, 0.4, 0.8];        % Modern blue
    colors.secondary = [0.3, 0.5, 0.9];      % Lighter blue
    colors.success = [0.2, 0.7, 0.4];        % Green
    colors.danger = [0.9, 0.3, 0.3];         % Red
    colors.warning = [0.9, 0.7, 0.2];        % Yellow/Orange
    colors.background = [0.98, 0.98, 0.99];  % Very light gray
    colors.panel = [1, 1, 1];                % White
    colors.text = [0.1, 0.1, 0.1];           % Dark gray
    colors.textLight = [0.5, 0.5, 0.5];      % Medium gray
    colors.border = [0.85, 0.85, 0.85];      % Light gray border
...
function handles = createTrialAndDataPanel(parent, handles, yPos, height)
    ...
    handles.clear_input_btn = uicontrol('Parent', panel, ...
                                       ...
                                       'String', 'X', ...
                                       'Units', 'normalized', ...
                                       'Position', [0.85, y, 0.06, rowHeight], ...
                                       'BackgroundColor', colors.danger, ...
                                       'ForegroundColor', 'white', ...
                                       'FontWeight', 'bold', ...
                                       'Callback', @clearInputFile);
    
    % File status - NO FontStyle property
...
function handles = createJointEditorPanel(parent, handles, yPos, height)
    ...
    for i = 1:7
        ...
        % Color code G coefficient
        labelColor = colors.primary;
        if i == 7, labelColor = colors.success; end
...
function handles = createCoefficientsPanel(parent, handles, yPos, height)
    ...
    uicontrol('Parent', panel, ...
              ...
              'String', '🔍', ...
              'Units', 'normalized', ...
              'Position', [0.02, searchY, 0.04, 0.10], ...
              'BackgroundColor', colors.panel);
    ...
    handles.search_edit = uicontrol('Parent', panel, ...
                                   ...
                                   'String', '', ...
                                   'Units', 'normalized', ...
                                   'Position', [0.07, searchY, 0.20, 0.10], ...
                                   'BackgroundColor', 'white', ...
                                   'FontSize', 9, ...
    ...
    handles.clear_search_button = uicontrol('Parent', panel, ...
                                           ...
                                           'String', 'Clear', ...
                                           'Units', 'normalized', ...
                                           'Position', [0.28, searchY, 0.08, 0.10], ...
                                           'BackgroundColor', colors.danger, ...
                                           'ForegroundColor', 'white', ...
                                           'FontWeight', 'bold', ...
                                           'Callback', @clearSearch);
    
    % Control buttons with valid field names
    buttonY = 0.76;
    buttonHeight = 0.09;
    buttonWidth = 0.13;
    buttonSpacing = 0.01;
    
    % Button configuration: {display_text, field_name, color, callback}
    buttons = {
