...
function handles = createBatchSettingsPanel(parent, handles, yPos, height)
    ...
    handles.enable_checkpoint_resume = uicontrol('Parent', panel, ...
                                               'Style', 'checkbox', ...
                                               'String', 'Resume from checkpoint (if available)', ...
                                               'Value', 1, ...
                                               'Units', 'normalized', ...
                                               'Position', [0.02, y, 0.45, rowHeight], ...
                                               'BackgroundColor', colors.panel, ...
                                               'FontSize', 9, ...
                                               'TooltipString', 'When checked, automatically resume from existing checkpoint. When unchecked, always start fresh.');
    ...
    handles.clear_checkpoint_button = uicontrol('Parent', panel, ...
                                              ...
                                              'Units', 'normalized', ...
                                              'Position', [0.48, y, 0.20, rowHeight], ...
                                              'BackgroundColor', colors.danger, ...
                                              'ForegroundColor', 'white', ...
                                              'FontSize', 9, ...
...
function clearAllCheckpoints(src, event)
    try
        ...
        % Confirm deletion
        checkpoint_list = strjoin(checkpoint_files, '\n');
        answer = questdlg(sprintf('Delete the following checkpoint files?\n\n%s\n\nThis will force a fresh start on next run.', checkpoint_list), ...
        ...
        if strcmp(answer, 'Yes, Delete')
            ...
            if deleted_count > 0
                msgbox(sprintf('Successfully deleted %d checkpoint file(s).\nNext run will start fresh.', deleted_count), ...
                       'Checkpoints Cleared', 'help');
            else
                msgbox('No checkpoint files were deleted.', 'No Action', 'warn');
            end
        end
        
    catch ME
        errordlg(sprintf('Error clearing checkpoints: %s', ME.message), 'Error');
    end
end
