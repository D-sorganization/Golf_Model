...
class MotionCaptureLoader:
    ...
    def _extract_trajectories(self):
        ...
        club_pos_cols = [
            col
            for col in self.mocap_data.columns
...
class SimulationDataProcessor:
    ...
    def load_trial_data(self):
        ...
        if not csv_files:
            raise FileNotFoundError(
                f"No trial CSV files found in {self.data_directory}"
            )
            ...
            for pattern in patterns:
                if "*" in pattern:
                    ...
                    matching_cols.extend(
                        [col for col in df.columns if pattern_regex in col]
                    )
                else:
    ...
    def extract_training_data(self, df):
        ...
        input_cols = [
            ...
            and any(col.endswith(f"_{coeff}") for coeff in self.coefficient_names)
        ]
    ...
    def _extract_hand_trajectory(self, trial_data):
        ...
        if len(mp_pos_cols) >= 3:
            ...
            time_data = (
                trial_data["time"].values
                if "time" in trial_data.columns
            ...
            lh_cols = [
                col
                for col in trial_data.columns
                if "LHCalcsLogs_LeftHandPostion" in col
            ]
            rh_cols = [
                col
                for col in trial_data.columns
                if "RHCalcsLogs_RightHandPostion" in col
            ]
        ...
        return {"position": position, "velocity": velocity, "time": time_data}
    ...
    def _extract_trajectory_features(self, trajectory):
        ...
        position_norm, velocity_norm = self._normalize_trajectory(
...
class GolfSwingSkeleton:
    ...
    def update_from_simulation_data(self, sim_data, time_idx=0):
        ...
        joint_mapping = {
            "hip": [
                "HipLogs_HipGlobalPosition_1",
                "HipLogs_HipGlobalPosition_2",
                "HipLogs_HipGlobalPosition_3",
            ],
            "left_hand": [
...
class MotionMatchingVisualizer:
    ...
    def setup_layout(self):
        ...
        self.ax_3d.set_title(
            "3D Golf Swing Visualization", fontsize=14, fontweight="bold"
        )
    ...
    def setup_controls(self):
        ...
        self.time_slider = Slider(
            slider_ax, "Time", 0, 1, valinit=0, facecolor="lightblue", alpha=0.7
        )
        ...
        self.play_button = Button(
            play_ax, "Play", color="lightgreen", hovercolor="green"
        )
        ...
        self.reset_button = Button(
            reset_ax, "Reset", color="lightcoral", hovercolor="red"
        )
        ...
        self.display_options = CheckButtons(
            options_ax,
            ["Show Skeleton", "Show Trajectories", "Show Error"],
            [True, True, True],
        )
    ...
    def load_motion_capture_data(self, mocap_data):
        ...
        print(
            f"Loaded motion capture data with {len(mocap_data['hand_trajectory'])} time points"
        )
    ...
    def plot_trajectories(self, current_idx):
        ...
        if len(mocap_traj) > 1:
            ...
            self.ax_xy.scatter(
                mocap_traj[-1, 0], mocap_traj[-1, 1], c="blue", s=100, marker="o"
            )
            ...
            self.ax_xz.scatter(
                mocap_traj[-1, 0], mocap_traj[-1, 2], c="blue", s=100, marker="o"
            )
        ...
        if self.sim_data and "hand_trajectory" in self.sim_data:
            ...
            if len(sim_traj) > 1:
                ...
                self.ax_xy.scatter(
                    sim_traj[-1, 0], sim_traj[-1, 1], c="red", s=100, marker="s"
                )
                ...
                self.ax_xz.scatter(
                    sim_traj[-1, 0], sim_traj[-1, 2], c="red", s=100, marker="s"
                )
    ...
    def plot_error_analysis(self, current_idx):
        ...
        if min_length > 1:
            error = np.linalg.norm(
                mocap_traj[:min_length] - sim_traj[:min_length], axis=1
            )
            time_vec = np.linspace(0, 1, min_length)

            self.ax_error.plot(
                time_vec, error, "g-", linewidth=2, label="Position Error"
            )
            if min_length > 0:
...
class MotionMatchingSystem:
    ...
    def __init__(self, data_directory=".", results_directory="motion_matching_results"):
        ...
        print("Motion Matching System initialized")
    ...
    def train_neural_network(self, epochs=200, batch_size=32, learning_rate=0.001):
        ...
        if self.sim_data is None:
            raise ValueError(
                "No simulation data loaded. Call load_simulation_data() first."
            )
        ...
        for epoch in range(epochs):
            ...
            if val_loss < best_val_loss:
                ...
                torch.save(
                    model.state_dict(),
                    os.path.join(self.results_directory, "best_model.pth"),
                )
            else:
                patience_counter += 1
        ...
        model.load_state_dict(
            torch.load(os.path.join(self.results_directory, "best_model.pth"))
        )
        ...
        torch.save(
            model.state_dict(), os.path.join(self.results_directory, "final_model.pth")
        )
        joblib.dump(
            scaler_input, os.path.join(self.results_directory, "scaler_input.pkl")
        )
        joblib.dump(
            scaler_output, os.path.join(self.results_directory, "scaler_output.pkl")
        )
        ...
        with open(
            os.path.join(self.results_directory, "training_metadata.json"), "w"
        ) as f:
            json.dump(training_metadata, f, indent=2)

        # Store trained model and scalers
        self.trained_model = model
        self.scalers = {"input": scaler_input, "output": scaler_output}

        # Plot training results
        self.plot_training_results(
            train_losses, val_losses, test_targets_orig, test_predictions_orig
        )
    ...
    def predict_coefficients_for_mocap(self):
        ...
        with torch.no_grad():
            ...
            coeffs = self.scalers["output"].inverse_transform(
                coeffs_scaled.cpu().numpy()
            )[0]
        ...
        for joint_name in joint_names:
            ...
            for coeff_name in coefficient_names:
                if coeff_idx < len(coeffs):
                    predicted_coefficients[joint_name][coeff_name] = float(
                        coeffs[coeff_idx]
                    )
                    coeff_idx += 1
        ...
        with open(
            os.path.join(self.results_directory, "predicted_coefficients.json"), "w"
        ) as f:
    ...
    def generate_matlab_coefficient_file(self, coefficients):
        ...
        with open(matlab_file, "w") as f:
            f.write(
                "% Auto-generated polynomial coefficients for golf swing simulation\n"
            )
            f.write(
                f"% Generated by Motion Matching System: {datetime.now().isoformat()}\n"
            )
    ...
    def plot_training_results(self, train_losses, val_losses, y_test, y_pred):
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(self.results_directory, "training_results.png"),
            dpi=300,
            bbox_inches="tight",
        )
        plt.show()
    ...
    def run_complete_workflow(self, wiffle_file_path):
        ...
        try:
            ...
            print(
                f"MATLAB coefficients: {self.results_directory}/predicted_coefficients.m"
            )
...
def main():
    ...
    parser.add_argument(
        "--data-dir", default=".", help="Directory containing trial CSV files"
    )
    parser.add_argument(
        "--results-dir",
        default="motion_matching_results",
    ...
    system = MotionMatchingSystem(
        data_directory=args.data_dir, results_directory=args.results_dir
    )
...
if __name__ == "__main__":
    ...
    try:
        # Initialize system
        system = MotionMatchingSystem(
            data_directory=".", results_directory="motion_matching_results"
        )
        ...
        if os.path.exists(wiffle_file):
            ...
            print(
                "Run with: python script.py --wiffle-file Wiffle_ProV1_club_3D_data.xlsx"
            )
        else:
