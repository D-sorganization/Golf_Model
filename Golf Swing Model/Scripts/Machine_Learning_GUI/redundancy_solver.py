...
class JointRedundancySolver:
    ...
    def __init__(self, kinematics_model):
        ...
        self.joint_weights = {
            ...
            'TranslationX': 1.0, 'TranslationY': 1.0, 'TranslationZ': 1.0,
            # Hip rotation - crucial for power generation
            'HipX': 0.9, 'HipY': 0.9, 'HipZ': 0.9,
            # Spine - important for torso rotation
            'SpineX': 0.8, 'SpineY': 0.7, 'Torso': 0.85,
            # Shoulders - moderate importance
            'LScapX': 0.6, 'LScapY': 0.6, 'LSX': 0.7, 'LSY': 0.7, 'LSZ': 0.7,
            'RScapX': 0.6, 'RScapY': 0.6, 'RSX': 0.7, 'RSY': 0.7, 'RSZ': 0.7,
            # Arms - lower importance (more reactive)
            'LE': 0.5, 'LF': 0.4, 'LWX': 0.4, 'LWY': 0.4,
            'RE': 0.5, 'RF': 0.4, 'RWX': 0.4, 'RWY': 0.4
        }
        ...
        for n_specified in range(min_specified, max_specified + 1):
            ...
            for _ in range(n_configurations):
                ...
                for base_joint in self.joint_groups['base_motion']:
                    if base_joint not in specified_joints and len(specified_joints) < n_specified:
    ...
    def _evaluate_configuration(self, specified_joints, target_trajectory):
        ...
        coeffs, losses = optimizer.optimize(
            target_trajectory, n_iterations=50, lr=0.05
        )
    ...
    def _analyze_configuration(self, specified_joints):
        ...
        specified_importance = np.mean([self.joint_weights[j] for j in specified_joints])
        solved_importance = np.mean([self.joint_weights[j] for j in solved_joints]) if solved_joints else 0
    ...
    def visualize_redundancy_analysis(self, analysis_results, output_dir='redundancy_analysis'):
        """
        ...
        ax3 = axes[1, 0]
        self._plot_group_representation(ax3, analysis_results['analysis']['group_analysis'])
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'redundancy_analysis.png'), 
        ...
        with open(config_file, 'w') as f:
            json.dump({
                'specified_joints': analysis_results['best_configuration']['specified_joints'],
    ...
    def _plot_importance_distribution(self, ax, analysis):
        ...
        specified_importances = [self.joint_weights[j] for j in analysis['specified_joints']]
...
class AdaptiveRedundancyOptimizer:
    ...
    def optimize_with_adaptive_redundancy(self, target_trajectory, 
                                        initial_specified_joints=None,
                                        n_adaptations=3):
        """
    ...
    def _analyze_joint_errors(self, coefficients, target_trajectory):
        ...
        for joint in coefficients.keys():
            ...
            error_contribution = np.mean(np.linalg.norm(
                baseline_positions - test_positions, axis=1
            ))
            
            joint_errors[joint] = {
                'error_contribution': error_contribution,
                'coefficient_magnitude': np.mean([abs(coefficients[joint].get(c, 0)) 
                                                for c in 'ABCDEFG'])
            }
    ...
    def _adapt_configuration(self, current_specified, joint_errors):
        ...
        error_rankings = sorted(
            joint_errors.items(),
            key=lambda x: x[1]['error_contribution'],
            reverse=True
        )
        ...
        print(f"Adapted configuration: {len(current_specified)} -> {len(new_specified)} joints")
...
def demonstrate_redundancy_solver():
    ...
    for result in adaptive_results['all_results']:
        print(f"   Adaptation {result['adaptation']}: Loss = {result['final_loss']:.6f}")
