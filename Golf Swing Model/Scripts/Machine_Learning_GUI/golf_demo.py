...
class MotionAnalyzer:
    ...
    def analyze_joint_contributions(self, coefficients, target_trajectory):
        ...
        sorted_joints = sorted(contributions.keys(), 
    ...
    def plot_joint_analysis(self, coefficients, mocap_data, output_dir='analysis_results'):
        """
        ...
        velocities = self.compute_joint_velocities(coefficients, 
                                                 time_duration=mocap_data['time'][-1])
        ...
        contributions = self.analyze_joint_contributions(coefficients, 
                                                       mocap_data['midhand_positions'])
        ...
        plt.savefig(os.path.join(output_dir, 'joint_analysis.png'), dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _plot_joint_velocities(self, ax, velocities):
        ...
        sorted_joints = sorted(velocities.keys(), 
                             key=lambda j: velocities[j]['max_velocity'], 
                             reverse=True)[:6]
        
        for joint in sorted_joints:
            vel = velocities[joint]['velocity']
            ax.plot(time_vec, np.rad2deg(vel), label=f"{joint} (max: {velocities[joint]['max_velocity']:.1f}°/s)", 
                   linewidth=2)
    ...
    def _plot_coefficient_magnitudes(self, ax, coefficients):
        ...
        ax.barh([joint_names[i] for i in sorted_indices],
    ...
    def _plot_contribution_pie(self, ax, contributions):
        ...
        # Add "Others" category
        other_contrib = sum(contributions[j]['mean_contribution'] 
                           for j in contributions if j not in sorted_joints)
        if other_contrib > 0:
    ...
    def _plot_polynomial_orders(self, ax, coefficients):
        ...
        ax.set_xticklabels(['A (0)', 'B (1)', 'C (2)', 'D (3)', 'E (4)', 'F (5)', 'G (6)'])
...
def run_complete_demo():
    ...
    contributions = analyzer.analyze_joint_contributions(optimized_coeffs, 
                                                       mocap_data['midhand_positions'])
    
    # Create analysis plots
    analyzer.plot_joint_analysis(optimized_coeffs, mocap_data, 
...
def generate_summary_report(system, analyzer, coefficients, mocap_data, output_dir):
    ...
    with open(report_file, 'w') as f:
        ...
        f.write(f"Frame rate: {len(mocap_data['time'])/mocap_data['time'][-1]:.1f} Hz\n")
        
        # Hand trajectory statistics
        hand_pos = mocap_data['midhand_positions']
        f.write(f"\nHand midpoint trajectory:\n")
        f.write(f"  X range: [{hand_pos[:, 0].min():.3f}, {hand_pos[:, 0].max():.3f}] m\n")
        f.write(f"  Y range: [{hand_pos[:, 1].min():.3f}, {hand_pos[:, 1].max():.3f}] m\n")
        f.write(f"  Z range: [{hand_pos[:, 2].min():.3f}, {hand_pos[:, 2].max():.3f}] m\n")
        ...
        velocities = analyzer.compute_joint_velocities(coefficients, 
                                                     time_duration=mocap_data['time'][-1])
...
def create_matlab_simulation_script(coefficients, output_dir):
    ...
    with open(matlab_script, 'w') as f:
        f.write("% MATLAB script to run golf swing simulation with optimized coefficients\n")
