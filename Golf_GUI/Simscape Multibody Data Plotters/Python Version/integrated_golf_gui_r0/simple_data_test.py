                ...
                if not key.startswith('__'):  # Skip metadata
                    ...
                    if isinstance(value, np.ndarray):
                        ...
                        if value.ndim == 2 and value.shape[1] > 10:
                            print(f"    Large dataset: {value.shape[0]} rows, {value.shape[1]} columns")
    ...
    try:
        ...
        if 'BASEQ' in mat_data:
            ...
            if isinstance(baseq_data, np.ndarray) and baseq_data.ndim == 2:
                print(f"BASEQ data: {baseq_data.shape[0]} time points, {baseq_data.shape[1]} signals")
                ...
                if baseq_data.shape[1] > 50:
                    ...
                    for i in range(min(10, baseq_data.shape[1])):
                        ...
                        print(f"  Signal {i}: range [{col_data.min():.3f}, {col_data.max():.3f}], mean {col_data.mean():.3f}")
...
def check_required_signals():
    ...
    try:
        ...
        required_signals = ['CHx', 'CHy', 'CHz', 'MPx', 'MPy', 'MPz']  # Clubhead and Midpoint positions
        
        print("Checking for required signals in each dataset:")
        
        datasets = [('BASEQ', baseq_data), ('ZTCFQ', ztcfq_data), ('DELTAQ', delta_data)]
        ...
        for name, data in datasets:
            if 'BASEQ' in data or 'ZTCFQ' in data or 'DELTAQ' in data:
                ...
                if key in data:
                    dataset = data[key]
                    print(f"  {name}: {dataset.shape[0]} time points, {dataset.shape[1]} signals")
...
def main():
    ...
    if all_passed:
        ...
        print("4. 🔧 Consider adding GUI option to disable Simscape Results Explorer for speed")
