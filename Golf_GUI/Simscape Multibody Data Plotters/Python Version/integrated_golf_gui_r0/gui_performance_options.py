...
class PerformanceOptionsDialog:
    ...
    def create_dialog(self):
        ...
        self.simscape_info = ttk.Label(main_frame, 
        ...
        fast_restart_check = ttk.Checkbutton(main_frame, 
                                            text="Enable Fast Restart", 
                                            variable=self.fast_restart_var)
        fast_restart_check.grid(row=5, column=0, columnspan=2, sticky=tk.W, pady=5)
        
        fast_restart_info = ttk.Label(main_frame, 
    ...
    def update_simscape_info(self):
        ...
        if self.simscape_var.get():
            self.simscape_info.config(text="✅ Provides ~5% speed improvement\n   Reduces memory usage during simulation",
                                     foreground="green")
        else:
            self.simscape_info.config(text="⚠️  Simscape Results Explorer enabled\n   May slow down simulation",
                                     foreground="orange")
