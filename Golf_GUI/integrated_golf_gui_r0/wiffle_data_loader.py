...
class WiffleDataConfig:
    ...
    def __post_init__(self):
        ...
        if self.prov1_columns is None:
            self.prov1_columns = {
                'time': 'Time',
                'clubhead_x': 'CHx', 'clubhead_y': 'CHy', 'clubhead_z': 'CHz',
                'butt_x': 'Bx', 'butt_y': 'By', 'butt_z': 'Bz',
                'midpoint_x': 'MPx', 'midpoint_y': 'MPy', 'midpoint_z': 'MPz',
                'left_wrist_x': 'LWx', 'left_wrist_y': 'LWy', 'left_wrist_z': 'LWz',
                'left_elbow_x': 'LEx', 'left_elbow_y': 'LEy', 'left_elbow_z': 'LEz',
                'left_shoulder_x': 'LSx', 'left_shoulder_y': 'LSy', 'left_shoulder_z': 'LSz',
                'right_wrist_x': 'RWx', 'right_wrist_y': 'RWy', 'right_wrist_z': 'RWz',
                'right_elbow_x': 'REx', 'right_elbow_y': 'REy', 'right_elbow_z': 'REz',
                'right_shoulder_x': 'RSx', 'right_shoulder_y': 'RSy', 'right_shoulder_z': 'RSz',
...
class WiffleDataLoader:
    ...
    def load_excel_data(self, filepath: str) -> Dict[str, pd.DataFrame]:
        ...
        try:
            ...
            return {
                'ProV1': prov1_processed,
                'Wiffle': wiffle_processed
            }
    ...
    def _process_sheet_data(self, df: pd.DataFrame, sheet_name: str) -> pd.DataFrame:
        ...
        if len(data_df.columns) >= 16:
            ...
            print(f"✅ Extracted position data from columns 2-4 (Mid-hands) for {sheet_name}")
        else:
            print(f"⚠️  Insufficient columns in {sheet_name}, using first 3 numeric columns")
            numeric_cols = data_df.select_dtypes(include=[np.number]).columns
            if len(numeric_cols) >= 3:
                processed_data['clubhead_x'] = pd.to_numeric(data_df[numeric_cols[0]], errors='coerce')
                processed_data['clubhead_y'] = pd.to_numeric(data_df[numeric_cols[1]], errors='coerce')
                processed_data['clubhead_z'] = pd.to_numeric(data_df[numeric_cols[2]], errors='coerce')
            else:
                print(f"⚠️  Insufficient numeric columns in {sheet_name}, creating dummy data")
        ...
        print(f"✅ Processed {sheet_name}: {processed_data.shape}, "
              f"time range: [{processed_data['time'].min():.3f}, {processed_data['time'].max():.3f}]")
        
        return processed_data
    
    def _create_body_part_estimates(self, processed_data: pd.DataFrame, sheet_name: str):
        ...
        print(f"📐 Created body part estimates for {sheet_name} based on clubhead position")
    ...
    def _create_dummy_data(self, num_frames: int) -> pd.DataFrame:
        ...
        for pos in ['butt', 'midpoint', 'left_wrist', 'left_elbow', 'left_shoulder', 
                   'right_wrist', 'right_elbow', 'right_shoulder', 'hub']:
            processed_data[f'{pos}_x'] = processed_data['clubhead_x'] + np.random.normal(0, 0.1, num_frames)
            processed_data[f'{pos}_y'] = processed_data['clubhead_y'] + np.random.normal(0, 0.1, num_frames)
            processed_data[f'{pos}_z'] = processed_data['clubhead_z'] + np.random.normal(0, 0.1, num_frames)
    ...
    def _apply_noise_filtering(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        position_columns = [col for col in df.columns if any(axis in col for axis in ['_x', '_y', '_z'])]
        ...
        for col in position_columns:
            if not df[col].isna().all():
                ...
                try:
                    filtered_data = savgol_filter(df[col].ffill(), 
                                                window_length, 3)
    ...
    def convert_to_gui_format(self, excel_data: Dict[str, pd.DataFrame]) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        """
    ...
    def _create_baseq_format(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        position_mappings = {
            'CHx': 'clubhead_x', 'CHy': 'clubhead_y', 'CHz': 'clubhead_z',
            'Bx': 'butt_x', 'By': 'butt_y', 'Bz': 'butt_z',
            'MPx': 'midpoint_x', 'MPy': 'midpoint_y', 'MPz': 'midpoint_z',
            'LWx': 'left_wrist_x', 'LWy': 'left_wrist_y', 'LWz': 'left_wrist_z',
            'LEx': 'left_elbow_x', 'LEy': 'left_elbow_y', 'LEz': 'left_elbow_z',
            'LSx': 'left_shoulder_x', 'LSy': 'left_shoulder_y', 'LSz': 'left_shoulder_z',
            'RWx': 'right_wrist_x', 'RWy': 'right_wrist_y', 'RWz': 'right_wrist_z',
            'REx': 'right_elbow_x', 'REy': 'right_elbow_y', 'REz': 'right_elbow_z',
            'RSx': 'right_shoulder_x', 'RSy': 'right_shoulder_y', 'RSz': 'right_shoulder_z',
            'Hx': 'hub_x', 'Hy': 'hub_y', 'Hz': 'hub_z'
        }
    ...
    def _create_deltaq_format(self, prov1_df: pd.DataFrame, wiffle_df: pd.DataFrame) -> pd.DataFrame:
        ...
        position_components = ['clubhead', 'butt', 'midpoint', 'left_wrist', 'left_elbow', 
...
def main():
    ...
    try:
        ...
        print(f"Time range: {excel_data['ProV1']['time'].min():.3f} - {excel_data['ProV1']['time'].max():.3f}")
