                ...
                if not key.startswith('__'):  # Skip metadata keys
                    ...
                    if isinstance(value, np.ndarray):
                        ...
                        if value.ndim == 2 and value.shape[1] > 10:
                            print(f"    Sample columns: {list(range(min(5, value.shape[1])))}")
                    else:
...
def test_data_loader():
    ...
    try:
        ...
        for df_name, df in [('BASEQ', baseq_df), ('ZTCFQ', ztcfq_df), ('DELTAQ', delta_df)]:
...
def test_frame_processor():
    ...
    try:
        ...
        for frame_idx in [0, processor.get_num_frames()//2, processor.get_num_frames()-1]:
            ...
            if isinstance(baseq_data, np.ndarray) and baseq_data.ndim == 2:
                ...
                if baseq_data.shape[1] > 10:
                    ...
                    for i in range(min(10, baseq_data.shape[1])):
                        ...
                        print(f"  Column {i}: range [{col_data.min():.3f}, {col_data.max():.3f}], mean {col_data.mean():.3f}")
...
def main():
    ...
    if all_passed:
        ...
        print("2. Consider adding a GUI option to disable Simscape Results Explorer for speed")
