function parallelDatasetGeneration(num_simulations, num_workers)
% parallelDatasetGeneration.m
% Parallel dataset generation using parsim for maximum performance
% 
% Inputs:
...
for sim_idx = 1:num_simulations
    ...
    % Store metadata in separate arrays for later retrieval
    % (We'll use array indices to match results)
...
for sim_idx = 1:num_simulations
    try
        ...
        if ~isempty(runIDs)
            ...
            % to track run IDs more carefully in parallel execution
            latest_run = Simulink.sdi.getRun(runIDs(end - num_simulations + sim_idx));
...
if successful_sims > 0
    ...
    filename = sprintf('parallel_dataset_%d_sims_%s.mat', successful_sims, timestamp);
