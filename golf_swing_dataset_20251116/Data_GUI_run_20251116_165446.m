...
%
% OUTPUT SETTINGS:
% Output folder: C:\Users\diete\Repositories\Golf_Model\golf_swing_dataset_20251116
...
% First trial coefficients (first 10): -26.380, 6.660, 13.910, 42.990, 43.180, -4.360, -3.480, 12.180, 44.480, 14.680
%
% =================================================================
...
% Ensure Constants folder is on MATLAB path
script_dir = fileparts(mfilename('fullpath'));
constants_dir = fullfile(fileparts(script_dir), 'Constants');

% Check if UIColors is available, if not add path
if isempty(which('UIColors'))
    if exist(constants_dir, 'dir')
        addpath(constants_dir);
    else
        error('Constants folder not found at: %s', constants_dir);
    end
end
...
try
    ...
    if isfield(handles, 'preferences')
        ...
        % Auto-load last config file if it exists
        if isfield(prefs, 'last_config_file') && exist(prefs.last_config_file, 'file')
            try
                fprintf('Auto-loading last config file: %s\n', prefs.last_config_file);
                ...
                % Skip legacy format with handles structure
                if isfield(config, 'handles')
                    fprintf('Skipping legacy config file format\n');
                else
                    ...
                    fprintf('Configuration auto-loaded successfully\n');
                end
            catch ME
                fprintf('Warning: Could not auto-load config file: %s\n', ME.message);
            end
        end
    end
...
% Set default path to Dataset Generator directory
default_path = fileparts(mfilename('fullpath'));
if isfield(handles.preferences, 'last_config_file') && exist(handles.preferences.last_config_file, 'file')
    [default_path, ~, ~] = fileparts(handles.preferences.last_config_file);
end

[filename, pathname] = uiputfile('*.mat', 'Save Configuration', fullfile(default_path, 'Config.mat'));
if filename ~= 0
    config = struct();
    ...
    config_file_path = fullfile(pathname, filename);
    save(config_file_path, 'config');

    % Save the config file path to preferences for next time
    handles.preferences.last_config_file = config_file_path;
...
default_path = fileparts(mfilename('fullpath'));
if isfield(handles.preferences, 'last_config_file') && exist(handles.preferences.last_config_file, 'file')
    [default_path, default_file, ~] = fileparts(handles.preferences.last_config_file);
    default_file = [default_file, '.mat'];
else
    default_file = 'Config.mat';
end

[filename, pathname] = uigetfile('*.mat', 'Load Configuration', fullfile(default_path, default_file));
if filename ~= 0
    try
...
default_input_file = fullfile(fileparts(mfilename('fullpath')), '..', '..', 'Model', 'Inputs_GolfSwing3D_Kinetic.mat');
% Also try absolute path
if ~exist(default_input_file, 'file')
    default_input_file = 'C:\Users\diete\Repositories\Golf_Model\matlab\Model\Inputs_GolfSwing3D_Kinetic.mat';
end
if exist(default_input_file, 'file')
    handles.selected_input_file = default_input_file;
    if isfield(handles, 'input_file_edit')
        set(handles.input_file_edit, 'String', default_input_file);
    end
    fprintf('Default input file set: %s\n', default_input_file);
else
    handles.selected_input_file = '';
    fprintf('Warning: Default input file not found at: %s\n', default_input_file);
end
...
for batch_idx = start_batch:num_batches
    ...
    try
        ...
        % CRITICAL: Use full paths so parallel workers can find the functions
        functions_dir = fullfile(fileparts(mfilename('fullpath')), 'functions');
        attached_files = {
            config.model_path, ...
            fullfile(functions_dir, 'runSingleTrial.m'), ...
            fullfile(functions_dir, 'processSimulationOutput.m'), ...
            fullfile(functions_dir, 'setModelParameters.m'), ...
            fullfile(functions_dir, 'setPolynomialCoefficients.m'), ...
            fullfile(functions_dir, 'extractSignalsFromSimOut.m'), ...
            fullfile(functions_dir, 'extractFromCombinedSignalBus.m'), ...
            fullfile(functions_dir, 'extractFromNestedStruct.m'), ...
            fullfile(functions_dir, 'extractLogsoutDataFixed.m'), ...
            fullfile(functions_dir, 'extractSimscapeDataRecursive.m'), ...
            fullfile(functions_dir, 'traverseSimlogNode.m'), ...
            fullfile(functions_dir, 'extractDataFromField.m'), ...
            fullfile(functions_dir, 'combineDataSources.m'), ...
            fullfile(functions_dir, 'addModelWorkspaceData.m'), ...
            fullfile(functions_dir, 'extractWorkspaceOutputs.m'), ...
            fullfile(functions_dir, 'resampleDataToFrequency.m'), ...
            fullfile(functions_dir, 'getPolynomialParameterInfo.m'), ...
            fullfile(functions_dir, 'getShortenedJointName.m'), ...
            fullfile(functions_dir, 'generateRandomCoefficients.m'), ...
            fullfile(functions_dir, 'prepareSimulationInputsForBatch.m'), ...
            fullfile(functions_dir, 'restoreWorkspace.m'), ...
            fullfile(functions_dir, 'loadInputFile.m'), ...
            fullfile(functions_dir, 'checkStopRequest.m'), ...
            fullfile(functions_dir, 'extractCoefficientsFromTable.m'), ...
            fullfile(functions_dir, 'shouldShowDebug.m'), ...
            fullfile(functions_dir, 'shouldShowVerbose.m'), ...
            fullfile(functions_dir, 'shouldShowNormal.m'), ...
            fullfile(functions_dir, 'mergeTables.m'), ...
            fullfile(functions_dir, 'logical2str.m'), ...
            fullfile(functions_dir, 'extractTimeSeriesData.m'), ...
            fullfile(functions_dir, 'extractConstantMatrixData.m')
            };
    ...
    if get(handles.use_simscape, 'Value')
        ...
        try
            if ~bdIsLoaded(model_name)
                % Suppress Simulink path warnings during model loading
                ...
                try
                    ...
                    lastwarn(''); % Clear any warnings that occurred
                    model_was_loaded = true;
                catch load_error
                    warning(warning_state); % Restore warnings before rethrowing
                    rethrow(load_error);
                end
