...
try
    ...
    elseif isnumeric(data_value)
        ...
        if isvector(data_value)
            % Vector data - create individual columns
            for i = 1:length(data_value)
                col_name = sprintf('%s_%d', signal_name, i);
                constant_signals{end+1} = struct('name', col_name, 'data', repmat(data_value(i), 1000, 1));
            end
        elseif ismatrix(data_value)
            % Matrix data - flatten to individual columns
            [rows, cols] = size(data_value);
            for i = 1:rows
                for j = 1:cols
                    col_name = sprintf('%s_%d_%d', signal_name, i, j);
                    constant_signals{end+1} = struct('name', col_name, 'data', repmat(data_value(i,j), 1000, 1));
                end
            end
        end
    end
catch ME
    fprintf('Debug: extractConstantMatrixData failed: %s\n', ME.message);
end
end
