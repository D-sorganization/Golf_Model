...
try
    ...
    if isfield(memory_info, 'MemUsedMATLAB') && isnumeric(memory_info.MemUsedMATLAB)
        % This MATLAB version uses MemUsedMATLAB
        if isfield(memory_info, 'MemAvailableAllArrays') && isnumeric(memory_info.MemAvailableAllArrays)
            % Calculate total as used + available
            total_memory = memory_info.MemUsedMATLAB + memory_info.MemAvailableAllArrays;
            if total_memory > 0
                system_info.total_memory_gb = total_memory / (1024^3);
                system_info.available_memory_gb = memory_info.MemAvailableAllArrays / (1024^3);
                system_info.memory_usage_percent = (memory_info.MemUsedMATLAB / total_memory) * 100;
            else
            ...
            % Fallback: can't calculate without available memory
            system_info.total_memory_gb = NaN;
            system_info.available_memory_gb = NaN;
            system_info.memory_usage_percent = NaN;
        end
    elseif isfield(memory_info, 'PhysicalMemory') && isstruct(memory_info.PhysicalMemory) && ...
            isfield(memory_info.PhysicalMemory, 'Total') && isnumeric(memory_info.PhysicalMemory.Total) && ...
            isfield(memory_info.PhysicalMemory, 'Available') && isnumeric(memory_info.PhysicalMemory.Available)
