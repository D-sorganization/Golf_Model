...
class WiffleDataLoader:
    ...
    def load_wiffle_excel(self, filepath, sheet_name=None):
        ...
        for i in range(min(20, len(df))):
            ...
            numeric_count = (
                df.iloc[i].apply(lambda x: isinstance(x, (int, float))).sum()
            )
        ...
        print(
            f"Time range: {self.time_data[0]:.3f} to {self.time_data[-1]:.3f} seconds"
        )
        print(
            f"Mid-hand position range: X[{self.midhand_positions[:, 0].min():.3f}, {self.midhand_positions[:, 0].max():.3f}]"
        )
...
class GolfSwingKinematics:
    ...
    def compute_positions(self, joint_angles, time_idx=None):
        ...
        # Hip orientation
        R_hip = self._euler_to_matrix(
            get_angle("HipX"), get_angle("HipY"), get_angle("HipZ")
        )
        ...
        positions["spine"] = positions["hip"] + orientations["hip"] @ np.array(
            [0, 0, self.segments["torso_length"] / 2]
        )

        # Upper torso
        R_torso = self._euler_to_matrix(0, 0, get_angle("Torso"))
        orientations["torso"] = orientations["spine"] @ R_torso
        positions["torso"] = positions["spine"] + orientations["spine"] @ np.array(
            [0, 0, self.segments["torso_length"] / 2]
        )

        # Shoulders
        positions["left_shoulder"] = positions["torso"] + orientations[
            "torso"
        ] @ np.array([-self.segments["shoulder_width"] / 2, 0, 0])
        positions["right_shoulder"] = positions["torso"] + orientations[
            "torso"
        ] @ np.array([self.segments["shoulder_width"] / 2, 0, 0])

        # Left arm chain
        R_lscap = self._euler_to_matrix(get_angle("LScapX"), get_angle("LScapY"), 0)
        R_ls = self._euler_to_matrix(
            get_angle("LSX"), get_angle("LSY"), get_angle("LSZ")
        )
        orientations["left_shoulder"] = orientations["torso"] @ R_lscap @ R_ls

        # Left elbow
        positions["left_elbow"] = positions["left_shoulder"] + orientations[
            "left_shoulder"
        ] @ np.array([0, -self.segments["upper_arm"], 0])
        ...
        positions["left_wrist"] = positions["left_elbow"] + orientations[
            "left_forearm"
        ] @ np.array([0, -self.segments["forearm"], 0])

        # Left hand
        R_lw = self._euler_to_matrix(get_angle("LWX"), get_angle("LWY"), 0)
        orientations["left_hand"] = orientations["left_forearm"] @ R_lw
        positions["left_hand"] = positions["left_wrist"] + orientations[
            "left_hand"
        ] @ np.array([0, -self.segments["hand"], 0])

        # Right arm chain (similar to left)
        R_rscap = self._euler_to_matrix(get_angle("RScapX"), get_angle("RScapY"), 0)
        R_rs = self._euler_to_matrix(
            get_angle("RSX"), get_angle("RSY"), get_angle("RSZ")
        )
        orientations["right_shoulder"] = orientations["torso"] @ R_rscap @ R_rs

        positions["right_elbow"] = positions["right_shoulder"] + orientations[
            "right_shoulder"
        ] @ np.array([0, -self.segments["upper_arm"], 0])
        ...
        positions["right_wrist"] = positions["right_elbow"] + orientations[
            "right_forearm"
        ] @ np.array([0, -self.segments["forearm"], 0])

        R_rw = self._euler_to_matrix(get_angle("RWX"), get_angle("RWY"), 0)
        orientations["right_hand"] = orientations["right_forearm"] @ R_rw
        positions["right_hand"] = positions["right_wrist"] + orientations[
            "right_hand"
        ] @ np.array([0, -self.segments["hand"], 0])
        ...
        positions["club_head"] = (
            positions["club_grip"] + self.segments["club_length"] * club_direction
        )

        # Hand midpoint (for matching)
        positions["hand_midpoint"] = (
            positions["left_hand"] + positions["right_hand"]
        ) / 2.0
...
class PolynomialCoefficientOptimizer:
    ...
    def optimize(
        self, target_trajectory, initial_coeffs=None, n_iterations=500, lr=0.01
    ):
        """
        ...
        for joint in self.specified_joints:
            if initial_coeffs and joint in initial_coeffs:
                init_vals = np.array(
                    [initial_coeffs[joint].get(c, 0) for c in "ABCDEFG"]
                )
                coeffs_params[joint] = torch.nn.Parameter(
                    torch.tensor(init_vals, dtype=torch.float32)
                )
            else:
                coeffs_params[joint] = torch.nn.Parameter(
                    torch.randn(self.n_coeffs) * 0.1
                )
        ...
        for iteration in range(n_iterations):
            ...
            for t in range(n_time):
                positions, _ = self.kinematics.compute_positions(
                    joint_angles, time_idx=t
                )
            ...
            if iteration % 50 == 0:
                print(
                    f"  Iteration {iteration}: Loss = {total_loss.item():.6f}, Position Error = {position_loss.item():.6f}"
                )
...
class GolfSwingVisualizer:
    ...
    def setup_controls(self):
        ...
        self.options = CheckButtons(
            options_ax, ["Skeleton", "Trajectories"], [True, True]
        )
    ...
    def load_coefficients(self, coefficients):
        ...
        for t in range(n_frames):
            positions, _ = self.kinematics_model.compute_positions(
                joint_angles, time_idx=t
            )
    ...
    def plot_skeleton(self, positions):
        ...
        for joint, pos in positions.items():
            ...
            self.ax_3d.scatter(
                pos[0], pos[1], pos[2], s=size, c=color, edgecolors="black", linewidth=1
            )
    ...
    def plot_trajectories(self, current_frame):
        ...
        if self.sim_positions:
            sim_hands = np.array(
                [p["hand_midpoint"] for p in self.sim_positions[: current_frame + 1]]
            )
            sim_club = np.array(
                [p["club_head"] for p in self.sim_positions[: current_frame + 1]]
            )
    ...
    def plot_2d_trajectories(self, current_frame):
        ...
        if len(mocap_hands) > 1:
            ...
            self.ax_traj.scatter(
                mocap_hands[-1, 0], mocap_hands[-1, 1], c="blue", s=100
            )
        ...
        if self.sim_positions and current_frame > 0:
            ...
            self.ax_traj.plot(
                sim_hands[:, 0], sim_hands[:, 1], "r--", linewidth=2, label="Simulation"
            )
            self.ax_traj.scatter(sim_hands[-1, 0], sim_hands[-1, 1], c="red", s=100)
        ...
        if self.sim_positions and current_frame > 0:
            ...
            self.ax_club.plot(
                sim_club[:, 0], sim_club[:, 2], "orange", linewidth=2, label="Sim Club"
            )
    ...
    def toggle_animation(self, event):
        ...
        if not self.is_playing:
            ...
            self.animation = animation.FuncAnimation(
                self.fig,
                animate,
                frames=int(self.time_slider.valmax),
                interval=50,
                repeat=True,
            )
        else:
...
class GolfSwingMotionMatching:
    ...
    def optimize_coefficients(self, initial_coeffs=None):
        ...
        self.optimized_coeffs, losses = optimizer.optimize(
            target_trajectory, initial_coeffs=initial_coeffs, n_iterations=300, lr=0.01
        )
    ...
    def save_results(self):
        ...
        with open(
            os.path.join(self.output_dir, "optimized_coefficients.json"), "w"
        ) as f:
...
def main():
    ...
    parser.add_argument(
        "--output", default="motion_matching_results", help="Output directory"
    )
...
if __name__ == "__main__":
    ...
    print("\nExample usage:")
    print(
        "python golf_swing_motion_matching.py --wiffle Wiffle_ProV1_club_3D_data.xlsx --trial trial_011_20250802_205003.csv"
    )
    print("\nThis system will:")
