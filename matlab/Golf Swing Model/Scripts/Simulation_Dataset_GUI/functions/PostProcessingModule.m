...
try
    % Check if Python is available and accessible
    ...
    % Test Python availability without system calls
    try
        % Try to use MATLAB's Python interface directly
    ...
    if ~python_available
        warning('Python not accessible via MATLAB interface. Falling back to MAT format.');
        save(strrep(output_file, '.pt', '.mat'), '-struct', 'batch_data');
        return;
    end
    ...
    try
        ...
        % Execute Python script using MATLAB's Python interface instead of system()
        try
            % Use MATLAB's Python interface directly
            py.eval(python_script);
            fprintf('PyTorch data saved to: %s\n', output_file);
        catch ME
            warning('PyTorch export via MATLAB Python interface failed: %s. Falling back to MAT format.', ME.message);
            save(strrep(output_file, '.pt', '.mat'), '-struct', 'batch_data');
        end
    catch ME
...
try
    ...
    try
        ...
        try
            ...
            warning('TensorFlow export via MATLAB Python interface failed: %s. Falling back to MAT format.', ME.message);
            save(strrep(output_file, '.h5', '.mat'), '-struct', 'batch_data');
        end
    catch ME
...
try
    ...
    try
        ...
        try
            ...
            warning('NumPy export via MATLAB Python interface failed: %s. Falling back to MAT format.', ME.message);
            save(strrep(output_file, '.npz', '.mat'), '-struct', 'batch_data');
        end
    catch ME
...
try
    ...
    try
        ...
        try
            ...
            warning('Pickle export via MATLAB Python interface failed: %s. Falling back to MAT format.', ME.message);
            save(strrep(output_file, '.pkl', '.mat'), '-struct', 'batch_data');
        end
    catch ME
